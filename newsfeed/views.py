...
def newsfeed_view(request):
    ...
    for activity in newsfeed_activities:
        if activity.activity_type == POST or activity.activity_type == REBLOG:
            activities_w_extra_info.append({"activity":activity, "notes": activity.object.find_notes(), "is_liked": activity.object.is_liked_by(request.user.blog), "tags": activity.object.tags.all()})
        else:
